...
const rowTemplate = (todo: ToDo)  => html`
    <tr id=${`tr-${todo.id}`} @click=${() => toDoClicked(todo)}><td>${todo.id}</td><td>${todo.title}</td></tr>
`
...
class TodoTableComponent extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        store
            ...
            .subscribe(
                viewModel => render(tableTemplate(viewModel), this.shadowRoot)
        ) 
    }
}
